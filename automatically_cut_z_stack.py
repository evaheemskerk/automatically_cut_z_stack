...
for item in os.listdir(file_path):
    ...
    if len(cut_start_zeros) == 1 and len(cut_end_zeros) == 1 and cut_start_zeros[0]<cut_end_zeros[0]:
        ...
        not_cut.append(item)
        print(f'For file {file}, the cutting did not work. It suggested: cut start {cut_start_zeros} and cut end {cut_end_zeros}. Or: cut start {cut_start_high} and cut end {cut_end_high}')
    ...
    if len(not_cut)>0:
        ...
        if go_on == 'Y':
            for item in not_cut:
                ...
                start_stack = int(input("The first stack were the cut needs to be, is at: "))
                end_stack = int(input("The last stack were the cut needs to be, is at: "))
